...
class LoginForm(forms.Form):
    # username字段，label='用户名' 显示中文，默认会有一个参数required=True,说明字段不能为空,attrs={'class': 'form-control'}直接在这里添加css样式
    username_or_email = forms.CharField(label='用户名或邮箱   ', \
                               widget=forms.TextInput( \
                                            attrs={'class': 'form-control', 'placeholder': '请输入用户名或邮箱'}))
    # password字段, widget=forms.PasswordInput,密码使用密文，定义生成的input标签类型,'placeholder': '请输入密码'，显示在文本框里的提示
...
class RegForm(forms.Form):
    username = forms.CharField(label='用户名',
                               ...
                               min_length=3,
                               widget=forms.TextInput(
                                            attrs={'class': 'form-control', 'placeholder': '请输入3-30位长度用户名'}))
    # password字段, widget=forms.PasswordInput,密码使用密文，定义生成的input标签类型,'placeholder': '请输入密码'，显示在文本框里的提示
